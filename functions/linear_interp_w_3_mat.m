function [ f_wx ] = linear_interp_w_3_mat( tyear, w, f, N, x_crit, x_max, xi)
    ...
    m1 = jbase == Nval;
        f_wx(m1) = f(Nval+1, 1, tyear + 1,4,xi);
    m2 = jbase ~= Nval;
        f_wx(m2) = (1-change(m2)).*f(jbase(m2)+1,1,tyear+1,4,xi) + ...
               change(m2).*f(jbase(m2)+2,1,tyear+1,4,xi);
